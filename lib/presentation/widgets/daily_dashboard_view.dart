          ...
          TextButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text(AppStrings.snackCopiato)),
              );
              Navigator.pop(ctx);
            },
            child: const Text(
                    ...
                    builder: (context, state) {
                      if (state is TaskLoading || state is TaskInitial) {
                        ...
                        final filteredTasks = state.tasks.where((task) {
                          final parsedDate =
                              DateTime.tryParse(task.dataInizio) ?? now;
                        ...
                        filteredTasks.sort((a, b) {
                          ...
                          if (!a.tuttoIlGiorno && b.tuttoIlGiorno) return 1;
                          final dateA =
                              DateTime.tryParse(a.dataInizio) ?? DateTime.now();
                          final dateB =
                              DateTime.tryParse(b.dataFine) ?? DateTime.now();
